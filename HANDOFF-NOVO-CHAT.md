...
- **Versão atual exibida:** 247, em `index.html`. Revisão visual em `styles-identity.css`; decisões e referências em `DESIGN.md`. Cada pilha possui 25 cartas. O último arquétipo é o Baralho Aleatório azul: cria uma receita por partida com 5 Peões Combinados distintos, materiais normais suficientes, 2 Arenas distintas (incluindo Arenas de Calamidade) e Efeitos aleatórios. Os créditos de arte permanecem em `CREDITOS.md`, mas não são exibidos nos detalhes das cartas.
...
- Versão 243 remove a dependência da presença remota para registrar resultados, envia os dois decks e persiste confirmações pendentes até o Supabase aceitá-las.
- Migração correspondente: `supabase/migration-243-reliable-results.sql`.
