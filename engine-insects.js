...
function resolveTremblingEarthCombat(attacker,defender,attackers,defenders,attackValue,defenseValue,before,version){
 if(version!==gameVersion)return false;clearAnimationMarks([...attackers,...defenders]);
 if(attackValue>defenseValue)defenders.slice().forEach(u=>destroy(u,attacker.owner,'combate'));
 else if(attackValue<defenseValue)attackers.slice().forEach(u=>{let defeated=destroy(u,defender.owner,'combate');if(defeated)resolveCandyZombieConversion(u,defender)});
 else[...attackers,...defenders].forEach(u=>destroy(u,u.owner===attacker.owner?defender.owner:attacker.owner,'combate'));
 logCombatResult(`Terra Tremula reuniu ${attackers.length} contra ${defenders.length} peões (${attackValue} × ${defenseValue}).`,before);state.animating=false;clearAction();render();checkWin();if(onlineMode)syncOnlineState();return true
}
function doTremblingEarthAttack(attacker,defender){
 ...
 let attackValue=attackers.reduce((sum,u)=>sum+effectiveAtk(u,false),0),defenseValue=defenders.reduce((sum,u)=>sum+effectiveAtk(u,true),0),before={1:state.players[1].score,2:state.players[2].score},version=gameVersion,combatLabel=`Terra Tremula: ${attackers.length} contra ${defenders.length} · ${attackValue} × ${defenseValue}`;
 state.animating=true;attackers.forEach(u=>{u.combatRole='attacker';u.combatDx=defender.col-u.col;u.combatDy=defender.row-u.row});defenders.forEach(u=>u.combatRole='defender');clearAction();render();hint(combatLabel);if(onlineMode)sendPacket({type:'visual',kind:'combat',attackers:attackers.map(u=>u.id),defender:defender.id,defenders:defenders.map(u=>u.id),tower:null,label:combatLabel});setTimeout(()=>resolveTremblingEarthCombat(attacker,defender,attackers,defenders,attackValue,defenseValue,before,version),760);return true
}
...
checkWin=function(){
 if(!state?.insectWinner){let won=baseInsectCheckWin();if(won){let flux=$('#winner-flux');if(flux)flux.textContent=state.onlineFluxDelta?`${state.onlineFluxDelta>0?'+':''}${state.onlineFluxDelta} FLUX`:onlineMode&&currentAccount?(state.onlineResultRecorded?'RESULTADO REGISTRADO':'Calculando variação de Flux…'):'';updateRematchAvailability()}return won}if(!state.insectVictoryReady){startDoomsdayVictoryAnimation();return true}let winner=state.insectWinner;if(typeof announceOnlineMatchResult==='function')announceOnlineMatchResult(winner,'doomsday');if(typeof reportOnlineMatchResult==='function')reportOnlineMatchResult(winner,'doomsday');let p=state.players[winner],dialog=$('#winner-dialog');dialog.classList.add('doomsday-victory');dialog.querySelector('.winner-crest').textContent='✺';$('#winner-name').textContent=`${p.name} · ${archetypes[p.archetype].name}`;$('#winner-score').textContent='Cinco calamidades convergiram: a Tragédia Âmbar encerrou o duelo.';let flux=$('#winner-flux');if(flux)flux.textContent=state.onlineFluxDelta?`${state.onlineFluxDelta>0?'+':''}${state.onlineFluxDelta} FLUX`:onlineMode&&currentAccount?(state.onlineResultRecorded?'RESULTADO REGISTRADO':'Calculando variação de Flux…'):'';if(typeof updateRematchAvailability==='function')updateRematchAvailability();if(!dialog.open)dialog.showModal();return true
};
