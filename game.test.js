...
const insects = fs.readFileSync('engine-insects.js', 'utf8');
...
const tieContext={};
vm.createContext(tieContext);
vm.runInContext(source.match(/function tieCombatMessage\([^\n]+/)[0],tieContext);
assert.equal(tieContext.tieCombatMessage(350,[],[{name:'Peão A'}]),'Empate em 350: todos os participantes foram destruídos.');
assert.equal(tieContext.tieCombatMessage(350,[{name:'Peão protegido'}],[{name:'Peão destruído'}]),'Empate em 350: Peão protegido sobreviveu por efeitos de proteção; os demais participantes foram destruídos.');
assert.doesNotMatch(tieContext.tieCombatMessage(350,[{name:'Peão protegido'}],[{name:'Peão destruído'}]),/Diviníssimo/,'o registro não deve atribuir toda proteção ao Diviníssimo');
...
const drawContext = {state: {awaitingDraw: true, players: {1: drawPlayer}}, botActor: () => 1, botGameAdvantageScore: () => 1, botFusionMissingCount: () => 0, takePawnFromDeck: player => player.pawnDeck.pop(), takeEffectFromDeck: player => player.effectDeck.pop(), log: () => {},Math:{random:()=>0,round:Math.round,min:Math.min}};
...
assert.match(source, /moveTargets\(u\)\.filter\(target=>!botLethalTerrain\(u,target\.r,target\.c\)\)/);
...
assert.match(page, /engine-ui\.js\?v=66/);
...
assert.match(source, /profileIconUrl\(account\?\.profileIcon\)/, 'o ícone online deve vir do perfil do jogador');
assert.match(styles, /\.player-crown \.match-profile-icon\{[^}]+background:var\(--profile-color/, 'a cor de fundo online deve vir do perfil');
assert.match(source, /movementOffsets\(inspected\)/, 'o alcance no tabuleiro deve respeitar a orientação do dono do peão');
assert.match(source, /inspected\.owner===perspective\?'friendly':'enemy'/, 'o alcance deve diferenciar aliado e adversário na visão atual');
assert.match(styles, /\.movement-range-overlay\.friendly/);
assert.match(styles, /\.movement-range-overlay\.enemy/);
assert.match(styles, /\.cell\.valid::before\{z-index:6\}/, 'as bolinhas de movimento válido devem permanecer acima do alcance inspecionado');
assert.match(styles, /\.piece \.dual-xadria-art\{inset:2px/);
assert.match(page, /id="home-brand"/);
assert.match(page,/id="match-room-code" class="match-room-code hidden" type="button"/,'o cabeçalho deve reservar um botão separado para o código online');
assert.match(source, /\$\('#home-brand'\)\.onclick=/);
assert.match(source,/activeRoom=onlineMode&&typeof roomCode==='string'&&roomCode\.length>0/,'o código deve aparecer somente durante partidas online com sala ativa');
assert.match(source,/roomBadge\.textContent=activeRoom\?`#\$\{roomCode\}`:''/,'o indicador deve exibir o código real da conexão');
assert.match(source,/\$\('#match-room-code'\)\.onclick=copyRoomCode/,'clicar no código não deve acionar o botão Confluxo');
assert.match(network,/navigator\.clipboard\.writeText\(code\)/,'o código deve ser copiado sem o caractere visual #');
...
assert.match(source, /board\.dataset\.p1=state\.players\[1\]\.visualArchetype\|\|state\.players\[1\]\.archetype/,'a zona de posicionamento deve usar a identidade visual sorteada');
...
assert.match(source, /key:'tunnel',boardFeature:true/, 'entradas de túnel devem abrir os detalhes da carta que as criou');
assert.match(source, /name:'Obstáculo NATURAL'/, 'obstáculos naturais devem explicar seu funcionamento');
assert.match(source, /shownEffect\.arena\|\|shownEffect\.boardFeature\?'':/, 'obstáculos inspecionados não devem mostrar o botão de jogar carta');
assert.match(source, /clearRangeSelection\(\);clearAction\(\);render\(\);hint\('Alcance removido; os detalhes continuam abertos\.'\)/, 'uma casa vazia deve limpar apenas o alcance e manter os detalhes');
assert.match(source, /allUnits\(\)\.find\(unit=>unit\.id===rangeSelectedUnitId\)/, 'o alcance deve ter seleção própria, independente dos detalhes');
assert.match(source, /function playUnitAnimation\(u,key\)/, 'animações de peões devem possuir controle para tocar apenas uma vez por evento');
assert.match(source, /playUnitAnimation\(u,'combatRole'\)/, 'combate não deve reiniciar ao renderizar novamente');
assert.match(source, /playUnitAnimation\(u,'deployed'\)/, 'entrada de peão não deve reiniciar ao clicar');
assert.match(source, /playUnitAnimation\(u,'fruitPickup'\)/, 'coleta de fruta não deve reiniciar ao clicar');
assert.match(source, /playStateAnimation\('initialDeal',state\.initialDeal\)/, 'animação da mão inicial deve tocar somente uma vez');
assert.match(source, /if\(rangeSelectedUnitId===u\.id\)rangeSelectedUnitId=null/, 'destruir um peão inspecionado deve limpar o alcance');
assert.match(source, /if\(spectatorSelected\?\.id===u\.id\)spectatorSelected=null/, 'destruir um peão inspecionado deve limpar seu alcance para espectadores');
...
assert.match(source, /cells\.filter\(target=>!botLethalTerrain\(u,target\.r,target\.c\)\)/, 'o bot não deve posicionar um peão vulnerável sobre terreno mortal');
assert.match(source, /moveTargets\(u\)\.filter\(target=>!botLethalTerrain\(u,target\.r,target\.c\)\)/, 'o bot não deve escolher movimentos sobre terreno mortal');
const botTerrainContext={pitAt:(r,c)=>r===1&&c===1,nuclearWasteAt:(r,c)=>r===2&&c===2};
vm.createContext(botTerrainContext);
vm.runInContext(source.match(/function botLethalTerrain\([^\n]+/)[0],botTerrainContext);
assert.equal(botTerrainContext.botLethalTerrain({types:['INSETO']},2,2),true,'lixo nuclear deve ser reconhecido como mortal');
assert.equal(botTerrainContext.botLethalTerrain({types:['RADIOATIVO']},2,2),false,'peões RADIOATIVOS podem ignorar o lixo nuclear');
assert.equal(botTerrainContext.botLethalTerrain({types:['RADIOATIVO']},1,1),true,'Poços continuam perigosos até para peões RADIOATIVOS');
...
assert.match(source, /Array\.from\(\{length:25\}/, 'os decks de Efeito devem ter 25 cartas');
assert.match(source, /kinds\.length<25/, 'os decks de Peões devem ser completados até 25 cartas');
assert.match(source, /function fusionNormalMaterialCounts\(/, 'a pilha deve reservar materiais normais para todas as combinações');
...
assert.match(source, /if\(!u\|\|!p\?\.units\?\.some\(piece=>piece\.id===u\.id\)\)return false/, 'um peão já removido não pode conceder pontos novamente');
assert.match(source, /\$\('#end-turn'\)\.onclick=\(\)=>endTurn\(\)/, 'o botão deve usar as regras adicionadas ao encerrar turno');
...
assert.equal(noticeContext.document.title, 'Confluxo');
...
assert.match(page, /<h4>Ataque em conjunto<\/h4>/);
assert.match(page, /outros peões em contato com o alvo somam ATK automaticamente/);
assert.match(page, /network\.js\?v=51/);
assert.match(page, /id="chat-form"/,'a crônica deve possuir composição de mensagens');
assert.match(page, /id="chat-input"[^>]+maxlength="160"/,'mensagens devem possuir limite visível');
assert.match(source, /function sendChatMessage\(\)/,'a partida deve enviar mensagens pelo formulário');
assert.match(source, /function escapeLogText\(value=''/,'mensagens do log devem escapar conteúdo HTML');
assert.match(styles, /log-entry\.log-chat/,'mensagens devem ser diferenciadas das ações da partida');
assert.match(page, /styles-game\.css\?v=16/);
assert.match(page, /id="winner-menu"/);
assert.match(insects, /\$\('#winner-menu'\)\.onclick=returnToMenuAfterMatch/);
...
assert.match(source, /!\[['"]eyes['"],['"]emberMummify['"]\]\.includes\(k\)&&effects\[k\]\.type===['"]EQUIPAMENTO['"]/);
...
arenaBotContext.state.arena=null;arenaBotContext.state.players[2].hand=['kingdom'];arenaBotLogs.length=0;
assert.equal(arenaBotContext.botPlayEffect(),true,'o Reino de Xadria sorteado diretamente no Aleatório deve poder ser jogado');
assert.match(arenaBotLogs[0].message,/Bot ativou a Arena Reino de Xadria/,'a ativação direta do Reino deve aparecer na crônica');
assert.match(source, /function botMatchupScore\(/);
assert.match(source, /function botProjectedEnemyRisk\(defender\)/);
assert.match(source, /moveTargets\(mover\)\.filter\(target=>!botLethalTerrain\(mover/);
...
assert.match(styles,/#winner-dialog\{width:min\(430px,calc\(100vw - 20px\)\);min-width:0;max-width:calc\(100vw - 20px\);max-height:calc\(100dvh - 20px\);overflow-x:hidden;overflow-y:auto/,'a animação de vitória não pode criar rolagem horizontal');
assert.match(styles,/#winner-dialog h2,#winner-dialog p\{overflow-wrap:anywhere\}/,'nomes e mensagens longas não podem alargar a janela de vitória');
...
assert.match(source, /filter\(entry=>!entry\.card\.fusion&&!entry\.card\.condition\)/);
assert.match(source, /drawRandomInitialPawns\(player\)/);
assert.doesNotMatch(source, /starters:/);
const initialDrawContext = {Math:{random:()=>0,floor:Math.floor}};
...
assert.doesNotMatch(page,/id="spectate-online"/,'o lobby não deve exigir um botão separado para espectadores');
assert.match(page,/id="quick-match-online"[^>]*>[\s\S]*?PARTIDA RÁPIDA/,'o lobby deve oferecer partida rápida');
assert.doesNotMatch(page,/Você será o Duelista [12]/,'o lobby não deve exibir papéis redundantes');
assert.match(network,/if\(dataChannel\)return attachSpectatorChannel\(channel,true\)/,'uma terceira pessoa deve entrar automaticamente como espectador');
assert.match(network,/packet\.type==='spectator-assigned'/,'o convidado deve reconhecer a promoção automática a espectador');
assert.match(network,/function startQuickMatch\(/,'a partida rápida deve possuir uma fila automática');
