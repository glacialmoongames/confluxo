...
assert.match(page, /engine-actions-b\.js\?v=26/);
assert.match(page, /engine-actions-a\.js\?v=30/);
assert.match(page, /VERSÃO 169/);
...
assert.match(page, /engine-ui\.js\?v=44/);
...
assert.match(page, /network\.js\?v=36/);
assert.match(page, /styles-game\.css\?v=12/);
...
assert.match(styles,/\.deck-choice\[data-deck=xadria\]\.selected\{border-color:#c45cff;background:linear-gradient\(145deg,#74269b,#2c103d\);box-shadow:0 0 22px #a832df66\}/,'Seleção de Xadria deve usar aro e brilho roxos, sem dourado');
...
assert.match(styles,/:root\{--arc-xadria-zone:linear-gradient\(145deg,#8528ad,#3f0c52\)\}/,'a zona Xadria deve usar roxo forte e saturado');
assert.match(styles,/\.board\[data-p1=xadria\] \.cell\.zone1\{background:var\(--arc-xadria-zone\);box-shadow:inset 0 -4px var\(--arc-xadria-border\)\}/,'o polo do J1 Xadria deve consumir a nova paleta');
assert.match(styles,/\.board\[data-arena=mausoleum\]\[data-p2=xadria\] \.cell\.mausoleum-zone-p2\{background:var\(--arc-xadria-zone\)!important\}/,'a expansão do Mausoléu deve preservar a zona Xadria saturada');
