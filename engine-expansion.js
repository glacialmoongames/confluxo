...
Object.assign(defs,{
 divinissimo:{name:'Diviníssimo da Casa Branca',atk:300,movement:[[-2,-2],[-2,2],[-1,-1],[-1,1],[1,-1],[1,1],[2,-2],[2,2]],types:['LUZ'],glyph:'✧',fusion:2,materials:{type:'LUZ'},text:'Enquanto estiver em campo, seus peões Xadria não são destruídos quando um combate termina empatado.'},
 terror:{name:'Terror Umbra da Casa Preta',atk:150,movement:[[-2,-1],[-2,1],[-1,-1],[-1,0],[-1,1],[0,-2],[0,-1],[0,1],[0,2],[1,-1],[1,0],[1,1],[2,-1],[2,1]],types:['TREVAS'],glyph:'☠',fusion:2,materials:{type:'TREVAS'},text:'Ganha 100 ATK para cada peão adversário em campo.'},
 atra:{name:'Atra da Casa Preta',atk:500,movement:[[-3,-3],[-3,3],[-2,-2],[-2,0],[-2,2],[-1,-1],[-1,0],[-1,1],[0,-2],[0,-1],[0,1],[0,2],[1,-1],[1,0],[1,1],[2,-2],[2,0],[2,2],[3,-3],[3,3]],types:['TREVAS'],glyph:'♛',fusion:4,materials:{type:'TREVAS'},activated:true,abilityLabel:'SACRIFICAR PEÃO',text:'Habilidade: sacrifique um aliado em seu raio para ganhar um movimento e um ataque extras neste turno.'},
 impoluto:{name:'Impoluto da Casa Branca',atk:400,movement:[[-2,0],[-1,-1],[-1,0],[-1,1],[0,-2],[0,-1],[0,1],[0,2],[1,-1],[1,0],[1,1],[2,0]],types:['TREVAS'],glyph:'♚',fusion:3,materials:{type:'LUZ'},activated:true,abilityLabel:'CONCEDER ATK',text:'Uma vez por turno, perde 100 ATK e concede 200 ATK permanentemente a outro peão em seu raio.'},
 ...
 creature:{name:'Criatura do Abismo',atk:200,movement:[[-1,-2],[-1,-1],[-1,0],[-1,1],[-1,2],[0,-1],[0,1],[1,-2],[1,-1],[1,0],[1,1],[1,2]],types:['TREVAS'],glyph:'●',text:'Um peão Abissal básico.'},
 devotee:{name:'Devoto do Abismo',atk:250,movement:[[-2,-1],[-2,1],[-1,-1],[-1,0],[-1,1],[1,-1],[1,0],[1,1]],types:['TREVAS'],glyph:'†',text:'Quando entra em campo, cria um Poço sem Fundo em qualquer casa livre.'},
 raven:{name:'Corvo do Abismo',atk:150,movement:[[-2,0],[-1,-1],[-1,1],[1,-1],[1,1],[2,0]],types:['TREVAS'],glyph:'⌃',text:'Quando qualquer jogador usa um Equipamento, cria uma cópia dessa carta na mão do dono do Corvo.'},
 amalgam:{name:'Amálgama do Abismo',atk:0,movement:[],types:['TREVAS'],glyph:'☍',fusion:2,variableFusion:true,materials:{archetype:'abyss'},text:'Combine dois ou mais peões Abissais. Pode incluir peões adversários que toquem um Devoto; soma o ATK e os alcances de todos os materiais.'},
 repugnium:{name:'Repugnium do Abismo',atk:400,movement:[[-2,-1],[-2,0],[-2,1],[-1,-2],[-1,-1],[-1,0],[-1,1],[-1,2],[0,-2],[0,-1],[0,1],[0,2],[1,-2],[1,-1],[1,0],[1,1],[1,2],[2,-1],[2,0],[2,1]],types:['TREVAS'],glyph:'◉',fusion:2,materials:{archetype:'abyss'},condition:'destroyed-this-turn',text:'Só pode ser combinado se um peão foi destruído neste turno. Inimigos em seu raio ficam presos e são destruídos após dois turnos nele.'},
 anssiedium:{name:'Anssiedium do Abismo',atk:500,movement:[[-2,-1],[-2,0],[-2,1],[-1,0],[1,0],[2,-1],[2,0],[2,1]],types:['TREVAS'],glyph:'▼',fusion:1,materials:{archetype:'abyss'},condition:'pit-this-turn',text:'Só pode ser combinado após um peão cair em um Poço neste turno. Ao se mover, deixa um Poço na casa anterior.'}
});
...
Object.assign(effects,{
 ...
 noEscape:{name:'Não Há Escapatória',type:'UTILIDADE',icon:'↯',text:'Nenhum peão pode se mover durante o próximo turno do oponente.'},
...
const cardIcons={infantry:'guards',tower:'white-tower',jester:'jester-hat',archer:'bowman',duck:'duck',horse:'horse-head',babel:'evil-tower',justice:'knight-banner',divinissimo:'chess-bishop',terror:'crowned-skull',atra:'chess-queen',impoluto:'chess-king',rabbit:'rabbit',rider:'mounted-knight',serpent:'cobra',hawk:'hawk-emblem',golem:'golem-head',monkey:'monkey',jaguar:'tiger-head',crocodile:'croc-jaws',creature:'monster-grasp',devotee:'cultist',raven:'raven',amalgam:'tentacles-skull',repugnium:'evil-eyes',anssiedium:'sinking-trap'};
Object.entries(cardIcons).forEach(([key,name])=>setCardIcon(defs[key],name,['tower','justice','divinissimo','impoluto'].includes(key)?'black':'white'));
...
archetypes.abyss={name:'Abissal',emblem:'◉',emblemArt:icon('evil-eyes'),pawns:['creature','devotee','raven'],fusions:['amalgam','repugnium','anssiedium'],effects:['abyss','noEscape','eyes','pit','push','reveal','peace']};
