-- Versão 247: libera o Baralho Aleatório no perfil, matchmaking, presença e histórico.
begin;
...
alter table public.profiles add constraint profiles_profile_color_check
  check (profile_color in ('xadria','wild','celestial','abyss','candy','gold','egyptian','insects','random'));
...
alter table public.quick_match_queue add constraint quick_match_queue_deck_check
  check (deck in ('xadria','wild','celestial','abyss','candy','gold','egyptian','insects','random'));
...
begin
  ...
  loop
    ...
    definition:=replace(definition,
      $old$'xadria','wild','celestial','abyss','candy','gold','egyptian','insects'$old$,
      $new$'xadria','wild','celestial','abyss','candy','gold','egyptian','insects','random'$new$);
    definition:=replace(definition,
      $old$'xadria'::text, 'wild'::text, 'celestial'::text, 'abyss'::text, 'candy'::text, 'gold'::text, 'egyptian'::text, 'insects'::text$old$,
      $new$'xadria'::text, 'wild'::text, 'celestial'::text, 'abyss'::text, 'candy'::text, 'gold'::text, 'egyptian'::text, 'insects'::text, 'random'::text$new$);
    execute definition;
  end loop;
end;
$$;
