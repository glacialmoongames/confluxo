-- Versao 239: permite os arquetipos atuais na Partida Rapida.
-- A migracao 222 tentou alterar indiretamente a funcao formatada pelo PostgreSQL,
-- mas a definicao ativa continuou limitada aos seis decks antigos.
...
begin
  ...
  if found then
    return jsonb_build_object(
      ...
      'roomCode', current_ticket.room_code,
      'role', current_ticket.role
    );
  end if;
  ...
  if found then
    ...
    set status = 'matched',
        ...
        role = 'host',
        matched_at = now()
    where ticket = opponent.ticket;

    insert into public.quick_match_queue (
      ticket,
      player_id,
      display_name,
      deck,
  ...
  insert into public.quick_match_queue (ticket, player_id, display_name, deck)
  values (p_ticket, auth.uid(), clean_name, p_deck);
