...
function accountPublicSnapshot(){return currentAccount?{id:currentAccount.id,username:currentAccount.username,wins:Number(currentAccount.wins)||0,losses:Number(currentAccount.losses)||0,deckUsage:safeDeckUsage(currentAccount.deckUsage)}:null}
function safeAccountSnapshot(value){if(!value||typeof value!=='object'||!value.id)return null;return{id:String(value.id),username:normalizeAccountName(value.username)||'Duelista',wins:Math.max(0,Number(value.wins)||0),losses:Math.max(0,Number(value.losses)||0),deckUsage:safeDeckUsage(value.deck_usage||value.deckUsage)}}
...
async function lookupAccountProfile(value){let candidate=safeAccountSnapshot(value);if(!candidate||!accountClient)return candidate;let {data,error}=await accountClient.from('profiles').select('id,username,wins,losses,deck_usage').eq('id',candidate.id).single();return error?null:safeAccountSnapshot(data)}
...
async function loadCurrentProfile(){
 ...
 let {data,error}=await accountClient.from('profiles').select('id,username,wins,losses,deck_usage').eq('id',session.user.id).single();if(error){currentAccount=null;setAccountMessage('A conta existe, mas o perfil ainda não foi preparado no banco.','error');updateAccountUI();return null}currentAccount=safeAccountSnapshot(data);updateAccountUI();return currentAccount
}
