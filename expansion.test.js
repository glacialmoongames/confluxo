...
for (const icon of ['half-body-crawling','skeleton','ghost','witch-face','vampire-cape','imp','graveyard','concrete-bag','wrapped-sweet','scroll-unfurled']) assert.ok(fs.existsSync(`assets/icons/${icon}.svg`),`ícone Mortos Doces ausente: ${icon}`);
...
assert.match(page, /engine-expansion\.js\?v=44/);
...
assert.match(expansion, /número de vezes igual à quantidade de peões aliados derrotados/);
...
assert.match(ui, /nativeArchetypeOfUnit\(u\)!=='abyss'&&!enemyAllowed/);
...
assert.doesNotMatch(expansion, /function makePawnDeck\(/, 'a expansão não deve substituir a composição central das pilhas');
assert.match(fs.readFileSync('engine-core.js','utf8'), /kinds\.length<25/);
assert.match(fs.readFileSync('engine-core.js','utf8'), /function randomVisualArchetype\(\)/);
assert.match(expansion, /player\?\.visualArchetype\|\|player\?\.archetype/,'mão e detalhes devem usar a cor sorteada do Aleatório');
