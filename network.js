...
function updateOnlineStart(){
 ...
 if(onlineRole==='host'){start.textContent=connected&&remoteDeck?'INICIAR PARTIDA ONLINE':connected?'SINCRONIZANDO DECKS…':'AGUARDANDO DUELISTA 2';start.disabled=!(connected&&remoteDeck)}else if(onlineRole==='spectator'){start.textContent='ASSISTINDO À SALA';start.disabled=true}else if(onlineRole==='quick'){start.textContent=quickMatchSearching?'PROCURANDO ADVERSÁRIO…':'PRONTO · PROCURAR PARTIDA';start.disabled=quickMatchSearching}else{start.textContent=connected?'AGUARDANDO O ANFITRIÃO':'ENTRE EM UMA SALA';start.disabled=true}
}
...
function startOnlineGame(){if(onlineRole==='quick')return startQuickMatch();if(onlineRole!=='host'||!dataChannel?.open||!remoteDeck)return;selectedPointGoal();networkStatus('Iniciando a partida…','connecting');document.querySelector('#setup').classList.add('hidden');newGame();sendGameState(true,true)}
