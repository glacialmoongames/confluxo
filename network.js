...
function announceOnlineMatchResult(winner,reason){if(!onlineMode||!localPlayer||onlineRole==='spectator'||!state?.matchId||state.onlineResultAnnounced||![1,2].includes(Number(winner)))return false;state.onlineResultAnnounced=true;let sent=sendPacket({type:'match-result',matchId:state.matchId,winner:Number(winner),reason:String(reason||'duelo').slice(0,40),player:localPlayer,account:typeof accountPublicSnapshot==='function'?accountPublicSnapshot():null});if(!sent)delete state.onlineResultAnnounced;return sent}
