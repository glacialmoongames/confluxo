var onlineMode=false,localPlayer=null,onlineRole=null,peerConnection=null,dataChannel=null,spectatorChannels=[],onlineSpectators=[],applyingRemote=false,remoteDeck=null,remoteName='',onlineNames={1:'Duelista 1',2:'Duelista 2'},onlineAccounts={1:null,2:null},networkSequence=0,networkRevision=0,roomCode='',reconnectTimer=null,heartbeatTimer=null,deckSyncTimer=null,stateRetryTimer=null,stateSyncTimer=null,remoteVisualTimer=null,presenceHeartbeatTimer=null,disconnectCheckTimer=null,disconnectStartedAt=0,presenceSessionId=null,reconnectAttempts=0,pendingStatePacket=null,lastPacketAt=0,stateChunks={},quickMatchTicket=null,quickMatchTimer=null,quickMatchSearching=false,quickMatchConnection=false,quickMatchStarting=false,rematchReady={1:false,2:false};
const announcedOnlineMatchResults=new Set();
const STATE_CHUNK_SIZE=8000;
const DISCONNECT_FORFEIT_MS=90000;
...
function updateRematchAvailability(){let button=document.querySelector('#winner-restart'),status=document.querySelector('#rematch-status');if(!button)return;let unavailable=onlineMode&&(onlineRole==='spectator'||!dataChannel?.open);button.disabled=unavailable||!!rematchReady[localPlayer];button.title=unavailable?'O outro duelista não está mais conectado.':'';button.textContent=rematchReady[localPlayer]?'PRONTO · AGUARDANDO':'Jogar novamente';if(status&&onlineMode&&localPlayer){let mine=!!rematchReady[localPlayer],other=!!rematchReady[localPlayer===1?2:1];status.textContent=mine&&other?'Os dois aceitaram · iniciando revanche…':mine?'Você está pronto · aguardando o adversário.':other?'O adversário está pronto para a revanche.':''}else if(status)status.textContent=''}
function resetRematchState(){rematchReady={1:false,2:false};updateRematchAvailability()}
function startReadyRematch(){if(onlineRole!=='host'||!rematchReady[1]||!rematchReady[2])return false;resetRematchState();document.querySelector('#winner-dialog')?.close();newGame();sendGameState(true,true);return true}
function requestOnlineRematch(){if(!onlineMode||!localPlayer||onlineRole==='spectator'||!dataChannel?.open||rematchReady[localPlayer])return false;rematchReady[localPlayer]=true;sendPacket({type:'rematch-ready',player:localPlayer,matchId:state?.matchId});updateRematchAvailability();startReadyRematch();return true}
function cleanRoomCode(value){return String(value||'').toUpperCase().replace(/[^A-Z0-9]/g,'').slice(0,12)}
function quickRoomCode(value=roomCode){return /^Q[A-Z0-9]{11}$/.test(cleanRoomCode(value))}
async function copyRoomCode(){let code=cleanRoomCode(roomCode),badge=document.querySelector('#match-room-code');if(!code)return false;try{await navigator.clipboard.writeText(code)}catch{let input=document.createElement('textarea');input.value=code;input.setAttribute('readonly','');input.style.position='fixed';input.style.opacity='0';document.body.append(input);input.select();let copied=document.execCommand?.('copy');input.remove();if(!copied)return false}if(badge){badge.classList.add('copied');badge.textContent='COPIADO';setTimeout(()=>{badge.classList.remove('copied');badge.textContent=`#${code}`},1100)}if(typeof hint==='function')hint(`Código ${code} copiado.`);return true}
function cleanPlayerName(value){return String(value||'').replace(/[<>]/g,'').trim().replace(/\s+/g,' ').slice(0,24)}
function safeChatPacket(packet,senderOverride=''){let message=typeof cleanChatMessage==='function'?cleanChatMessage(packet?.message):String(packet?.message||'').trim().slice(0,160),id=String(packet?.id||'').replace(/[^a-zA-Z0-9_-]/g,'').slice(0,80),sender=cleanPlayerName(senderOverride||packet?.sender)||'Duelista';if(!message||!id)return null;return{type:'chat',id,message,sender,player:Number(packet?.player)||0,turn:Math.max(1,Number(packet?.turn)||state?.turn||1)}}
function announceOnlineMatchResult(winner,reason){if(!onlineMode||!localPlayer||onlineRole==='spectator'||!state?.matchId||![1,2].includes(Number(winner)))return false;let key=`${state.matchId}:${localPlayer}`;if(announcedOnlineMatchResults.has(key))return false;let packet={type:'match-result',matchId:state.matchId,winner:Number(winner),reason:String(reason||'duelo').slice(0,40),player:localPlayer,account:typeof accountPublicSnapshot==='function'?accountPublicSnapshot():null};announcedOnlineMatchResults.add(key);let sent=sendPacket(packet);if(!sent){announcedOnlineMatchResults.delete(key);return false}state.onlineResultAnnounced=true;setTimeout(()=>{if(dataChannel?.open&&state?.matchId===packet.matchId)sendPacket(packet)},450);setTimeout(()=>{if(dataChannel?.open&&state?.matchId===packet.matchId)sendPacket(packet)},1400);return true}
...
async function touchMatchPresence(){if(!onlineMode||!localPlayer||!state?.matchId||!gameStarted()||typeof accountClient==='undefined'||!accountClient)return false;presenceSessionId??=makeQuickMatchTicket();let {data,error}=await accountClient.rpc('touch_match_presence',{p_match_id:state.matchId,p_player:localPlayer,p_session:presenceSessionId,p_name:state.players[localPlayer]?.name||onlineNames[localPlayer],p_deck:state.players[localPlayer]?.archetype||selectedDecks[localPlayer]});return !error&&data===true}
function startMatchPresence(){if(!onlineMode||!localPlayer||!state?.matchId||!gameStarted())return;presenceSessionId??=makeQuickMatchTicket();touchMatchPresence();if(!presenceHeartbeatTimer)presenceHeartbeatTimer=setInterval(touchMatchPresence,10000)}
function clearDisconnectForfeit(){clearTimeout(disconnectCheckTimer);disconnectCheckTimer=null;disconnectStartedAt=0}
function finishDisconnectForfeit(winner,recorded=false,fluxDelta=0){if(!state||state.forfeitWinner||state.celestialWinner)return false;clearDisconnectForfeit();state.forfeitWinner=winner;state.disconnectForfeit=true;state.onlineResultRecorded=recorded;let loser=winner===1?2:1;log(`${state.players[loser].name} permaneceu desconectado por mais de 90 segundos e perdeu a partida.`);document.body.classList.remove('online-waiting');render();checkWin();if(recorded&&typeof showOnlineFluxDelta==='function'&&fluxDelta)showOnlineFluxDelta(winner===localPlayer?Math.abs(fluxDelta):-Math.abs(fluxDelta));if(recorded&&typeof refreshOnlineAccounts==='function'){refreshOnlineAccounts();if(typeof loadLeaderboard==='function')loadLeaderboard();if(typeof currentAccount!=='undefined'&&currentAccount&&typeof hint==='function')hint('Resultado confirmado: a variação desta partida foi de 27 a 33 Flux.')}return true}
async function checkDisconnectForfeit(){disconnectCheckTimer=null;if(!onlineMode||!localPlayer||!state||dataChannel?.open||state.forfeitWinner)return clearDisconnectForfeit();let elapsed=Date.now()-disconnectStartedAt;if(elapsed<DISCONNECT_FORFEIT_MS){disconnectCheckTimer=setTimeout(checkDisconnectForfeit,Math.min(5000,DISCONNECT_FORFEIT_MS-elapsed));return}if(typeof accountClient!=='undefined'&&accountClient&&presenceSessionId){await touchMatchPresence();let {data,error}=await accountClient.rpc('claim_disconnect_win',{p_match_id:state.matchId,p_player:localPlayer,p_session:presenceSessionId});if(!error&&data?.status==='claimed')return finishDisconnectForfeit(localPlayer,true,Number(data.fluxDelta)||0);if(!error&&data?.status==='lost')return finishDisconnectForfeit(localPlayer===1?2:1,true,Number(data.fluxDelta)||0);if(!error&&data?.status==='waiting')networkStatus('Conexão interrompida, mas o adversário ainda está online','connecting');else networkStatus('Não foi possível confirmar a desconexão — tentando novamente…','connecting');disconnectCheckTimer=setTimeout(checkDisconnectForfeit,5000);return}finishDisconnectForfeit(localPlayer,false,0)}
function startDisconnectForfeit(){if(!onlineMode||!localPlayer||!state||!gameStarted()||state.forfeitWinner)return;if(!disconnectStartedAt)disconnectStartedAt=Date.now();startMatchPresence();clearTimeout(disconnectCheckTimer);disconnectCheckTimer=setTimeout(checkDisconnectForfeit,Math.max(250,DISCONNECT_FORFEIT_MS-(Date.now()-disconnectStartedAt)));networkStatus('Reconectando — após 90 segundos desconectado o jogador perde','connecting')}
function clearConnectionTimers(){clearTimeout(reconnectTimer);clearInterval(heartbeatTimer);clearInterval(deckSyncTimer);clearTimeout(stateRetryTimer);clearTimeout(stateSyncTimer);clearInterval(presenceHeartbeatTimer);clearTimeout(disconnectCheckTimer);reconnectTimer=null;heartbeatTimer=null;deckSyncTimer=null;stateRetryTimer=null;stateSyncTimer=null;presenceHeartbeatTimer=null;disconnectCheckTimer=null;disconnectStartedAt=0;presenceSessionId=null}
function closePeer(){
 clearConnectionTimers();reconnectAttempts=0;networkRevision=0;pendingStatePacket=null;lastPacketAt=0;stateChunks={};rematchReady={1:false,2:false};
...
function handlePeerError(error){
 ...
 let quickSpectatorRetry=error?.type==='peer-unavailable'&&quickRoomCode()&&['guest','spectator'].includes(onlineRole)&&reconnectAttempts<6;
 if((quickMatchConnection||quickSpectatorRetry||document.querySelector('#setup').classList.contains('hidden'))&&error?.type!=='browser-incompatible'){networkStatus(quickSpectatorRetry?'A sala rápida ainda está abrindo — tentando novamente…':'Reconectando a partida…','connecting');scheduleReconnect(quickSpectatorRetry?1200:undefined);return}
...
function updateQuickMatchButton(){let button=document.querySelector('#quick-match-online'),title=button?.querySelector('b'),hint=button?.querySelector('small');if(!button)return;button.classList.toggle('searching',quickMatchSearching);if(title)title.textContent=quickMatchSearching?'CANCELAR BUSCA':'PARTIDA RÁPIDA';if(hint)hint.textContent=quickMatchSearching?'Voltar para as salas':'Escolha o deck e confirme'}
function setLobbyBusy(busy){document.querySelector('#host-online').disabled=busy;document.querySelector('#join-online').disabled=busy;document.querySelector('#quick-match-online').disabled=busy&&!quickMatchSearching;document.querySelector('#generate-code').disabled=busy;document.querySelector('#room-code').disabled=busy;document.querySelector('#online-name').disabled=busy;updateQuickMatchButton()}
...
function attachChannel(channel){
 ...
 dataChannel=channel;
 dataChannel.on('open',()=>{if(channel!==dataChannel)return;clearTimeout(reconnectTimer);reconnectTimer=null;clearDisconnectForfeit();reconnectAttempts=0;lastPacketAt=Date.now();startHeartbeat();updateRematchAvailability();document.querySelector('#network-badge').classList.remove('hidden');document.body.classList.remove('connection-lost');if(onlineRole==='spectator'){networkStatus('Conectado como espectador','connected');sendPacket({type:'spectate-request'});return}networkStatus('Conectado ao outro duelista','connected');startDeckSync();updateOnlineStart();if(state&&gameStarted()){startMatchPresence();updateOnlineLock()}if(gameStarted()){flushPendingState();sendPacket({type:'sync-request',revision:networkRevision});if(onlineRole==='host')setTimeout(()=>sendGameState(true,false),80)}});
 dataChannel.on('data',raw=>{if(channel===dataChannel){if(raw?.type==='chat'){let packet=safeChatPacket(raw);if(!packet)return;if(onlineRole==='host')broadcastSpectators(packet);receivePacket(packet);return}if(onlineRole==='host'&&['visual','state','state-chunk'].includes(raw?.type))broadcastSpectators(raw);receivePacket(raw)}});
...
function handleChannelClose(channel){
 ...
 if(!onlineMode||channel!==dataChannel)return;clearInterval(heartbeatTimer);heartbeatTimer=null;dataChannel=null;updateRematchAvailability();networkStatus('Reconectando a partida…','connecting');document.body.classList.add('online-waiting','connection-lost');startDisconnectForfeit();scheduleReconnect()
}
...
function sendPacket(packet){
 ...
 try{
  ...
  dataChannel.send(packet);if(onlineRole==='host'&&['visual','chat'].includes(packet?.type))broadcastSpectators(packet);return true
...
function receivePacket(raw){
 ...
 if(packet.type==='chat'){let chat=safeChatPacket(packet);if(chat&&typeof appendChatMessage==='function')appendChatMessage(chat);return}
 if(packet.type==='match-result'){let reporter=Number(packet.player),winner=Number(packet.winner);if(packet.matchId!==state?.matchId||reporter==localPlayer||![1,2].includes(reporter)||![1,2].includes(winner))return;let account=typeof safeAccountSnapshot==='function'?safeAccountSnapshot(packet.account):null;state.onlineResultIdentity??={};state.onlineResultIdentity[reporter]=true;if(account){onlineAccounts[reporter]=account;if(state.players?.[reporter])state.players[reporter].account=account}announceOnlineMatchResult(winner,String(packet.reason||'duelo').slice(0,40));if(typeof reportOnlineMatchResult==='function')reportOnlineMatchResult(winner,String(packet.reason||'duelo').slice(0,40));return}
 ...
 if(packet.type==='rematch-ready'){let player=Number(packet.player);if(packet.matchId!==state?.matchId||![1,2].includes(player)||player===localPlayer)return;rematchReady[player]=true;updateRematchAvailability();startReadyRematch();return}
 ...
 let previousMatchId=state?.matchId;clearTimeout(remoteVisualTimer);remoteVisualTimer=null;applyingRemote=true;if(packet.started){quickMatchConnection=false;quickMatchStarting=false;document.querySelector('#setup').classList.add('hidden')}state=packet.state;if(previousMatchId&&state.matchId!==previousMatchId){resetRematchState();document.querySelector('#winner-dialog')?.close()}selectedDecks=packet.selectedDecks||selectedDecks;if(watching){spectatorSelected=findInspectedUnit(inspectedUnitId);spectatorEffect=inspectedEffect}else{selected=findInspectedUnit(inspectedUnitId);selectedEffect=inspectedEffect}mode=null;targets=[];pendingCard=null;castleFirst=null;fusionMaterials=[];startMatchPresence();
...
async function startQuickMatch(){if(quickMatchSearching)return;if(!accountClient)return networkStatus('A partida rápida precisa do serviço online. Recarregue a página e tente novamente.','error');selectedDecks[1]=selectedLobbyDeck();onlineNames[1]=getOnlineName('Duelista');quickMatchTicket=makeQuickMatchTicket();let ticket=quickMatchTicket;quickMatchSearching=true;setLobbyBusy(true);updateOnlineStart();networkStatus('Procurando um adversário…','connecting');let {data,error}=await accountClient.rpc('join_quick_match',{p_ticket:ticket,p_name:onlineNames[1],p_deck:selectedDecks[1]});if(ticket!==quickMatchTicket||!quickMatchSearching)return;if(error)return failQuickMatch(error);if(processQuickMatch(data))return;quickMatchTimer=setInterval(pollQuickMatch,1400)}
...
function cancelQuickMatch(){if(!quickMatchSearching)return false;stopQuickMatch();quickMatchConnection=false;quickMatchStarting=false;selectGameMode('online');networkStatus('Busca cancelada — escolha uma sala ou tente novamente','');return true}
