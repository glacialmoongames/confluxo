...
assert.match(ui,/#winner-restart'\)\.onclick=[^\n]+requestOnlineRematch\(\)/,'o botão de revanche deve marcar o jogador como pronto sem iniciar sozinho');
assert.match(network,/state\.matchId!==previousMatchId\)\{resetRematchState\(\);document\.querySelector\('#winner-dialog'\)\?\.close\(\)\}/,'o segundo jogador deve sair da tela de vitória quando a revanche começar');
assert.match(insects,/function lethalNuclearWasteAt\(/);
assert.match(insects,/function resolveNuclearWasteLanding\(/);
assert.match(ui,/pushTargets\(unit\)[^\n]+choice\.pit\|\|choice\.waste/,'Empurrão do bot deve reconhecer lixo nuclear como destino letal');
